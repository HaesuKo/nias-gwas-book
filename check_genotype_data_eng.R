# Basic QC reports for each breed
# ---------------------------------------------------
# ---- packages ----
...
for (breed in breeds) {
  ...
  p3 <- ggplot(hardy, aes(x = minuslog10P)) +
    ...
    labs(title = paste0(breed, ": HWE (-log10P)"),
         x = "-log10(P)", y = "Count")
  ...
  # failing SNPs in this genotype dataset
  n_fail_any <- sum(variant_qc$fail_any, na.rm = TRUE)
  # total SNPs in this genotype dataset (robust to any duplicates)
  n_snps_total <- dplyr::n_distinct(vmiss$ID)
  cat(sprintf("[%s] Total SNPs: %s; failing any filter: %s (%.2f%%)\n\n",
              breed,
